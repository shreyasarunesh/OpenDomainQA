...
class CreateIndex():
    ...
    def index(self, title, body, category, infobox, link, reference):
        ...
        words_set.update(title) # Index for title.
        for word in title:
            title_dict[word] += 1


        words_set.update(body)  # Index for body.
        for word in body:
            body_dict[word] += 1


        words_set.update(category)  # Index for category.
        for word in category:
            category_dict[word] += 1


        words_set.update(infobox)   # Index for infobox.
        for word in infobox:
            infobox_dict[word] += 1


        words_set.update(link)      # Index for wikipedia link.
        for word in link:
            link_dict[word] += 1


        words_set.update(reference)     # Index for references.
